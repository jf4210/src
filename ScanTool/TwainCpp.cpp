...
char libName[512];
	...
	USES_CONVERSION;
	m_hTwainDLL = LoadLibrary(A2T(libName));	//LoadLibrary
	if(m_hTwainDLL != NULL)
	{
		if(!(m_pDSMProc = (DSMENTRYPROC)GetProcAddress(m_hTwainDLL,MAKEINTRESOURCEA(1))))	//MAKEINTRESOURCE
		{
...
//		BOOL ret_value=SetCapability(CAP_DUPLEXENABLED,duplex,TRUE);
		if (duplex == 1)				//Ë«ÃæÉ¨Ãè
			duplex = TWTY_BOOL;
		BOOL ret_value = SetOneValueCapability(CAP_DUPLEXENABLED, duplex, TRUE);	//µ¥Ë«ÃæÉ¨Ãè¿ØÖÆ
...
void CTwain::TransferImage()
{
	TW_IMAGEINFO info;
	BOOL bContinue = TRUE;
	int nScanStatus = 0;		//É¨Ãè½áÊøÊ±×´Ì¬£¬1--Õı³£É¨Ãè½áÊø
	TRACE("********** TransferImage ************\n");
	while(bContinue)
	{
